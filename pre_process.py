    ...
    if response and 'success' in response:
        ...
        for org in orgs:
            ...
            if filename and org_slug:
                new_string  = "\norganization/{f}/     /organization/{s}/   301!".format(f=filename, s=org_slug)
            ...
            file_name, file_ext = os.path.splitext(org_logo)
            org_logo_filename = u"{l}{id}_org_logo{e}".format(l=logo_dir, id=filename, e=file_ext)
    ...
    if response and 'success' in response:
        ...
        for ser in sers:
            ...
            if filename and ser_slug:
                new_string  = "\nservice/{f}/     /service/{s}/   301!".format(f=filename, s=ser_slug)
                rewrites_string = rewrites_string + new_string
