...
function recvNearbyOrgs(results, whichCase, found_at_parent) {
    return { type: types.RECV_SEARCH_RESULTS_SUCCESS, case: whichCase, results, found_at_parent }
}
...
export function fetchNearbyOrgs(lat, lng, excludeAreaId, service_template_id,service_text) {
    return async (dispatch, getState) => {
        dispatch(clearAll());
        dispatch(requestSearchResults());
        let params = {
        ...
        try {
            ...
            if (resp && resp.success) {
                dispatch(recvSearchResults(resp.results, resp.case, resp.is_redirect, resp.found_at_parent));
            }
            else {
                dispatch(failedRecvSearchResults());
            }
