    ...
    componentDidMount(){
        ...
        if (allfaq.length > 0) {
            qaList = allfaq.map((qa, index) => {
                ...
                return <Fragment>
                    <ListItem disableGutters>
                        <ListItemText primary={<Typography component="h3" style={{ fontWeight: 500, color: "rgba(30, 30, 50, 1.40)", lineHeight: '1.5rem'}} variant='subheading'>{question}</Typography>} secondary={<Typography color="textPrimary">{text}</Typography>} />
                    </ListItem>
