...
class LandingDesktop extends React.Component {
  ...
  handleTabChange(e, newTabIdx){
    console.log(e,  newTabIdx);

    this.setState({
      selectedServiceIndex: newTabIdx
    });
  }
  ...
  render() {
    const { createSubInProgress, uploadable, sers } = this.props;

    console.log(sers, "landing sers desktop");
    ...
    const tabComp = (
      ...
      >
        {sers.map((s, idx) => (
          <StyledTab label={s.node.name} {...a11yProps(0)} />
        ))}
      </StyledTabs>
    );


    const filteredSers = sers.filter((s, index) => {
       return this.state.selectedServiceIndex === index
    });
      ...
      <div className={styles.columns}>
        ...
        >
          <div
            ...
            style={{ boxShadow: "0 .25rem 1rem rgba(48,55,66,.15)" }}
          >
            <div
              className={`${styles.column} ${styles.col12} ${styles.textCenter}`}
              style={{ margin: "1rem 0" }}
